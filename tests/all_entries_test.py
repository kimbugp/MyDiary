...
class all_entries_test(unittest.TestCase):
    ...
    def test_API_can_create_new_entries(self):
        ...
        response = test_user.post(
            '/api/v1/entries', data=json.dumps(test_entry), content_type="application/json")
        self.assertIn('Test Content', str(
            response.data), msg="all entries valid")
        self.assertEqual(response.status_code, 201)

    def test_API_get_one_entry(self):
        # Tests to show one  entry
        test_user = app.test_client(self)
        test_user.post('/api/v1/entries', data=json.dumps(test_entry),
                       content_type="application/json")
        response = test_user.get(
            '/api/v1/entries/1', data=json.dumps(test_entry), content_type='application/json')
        self.assertEqual(response.status_code, 200)
    ...
    def test_for_wrong_single_entry(self):
        ...
        test_user.post('/api/v1/entries', data=json.dumps(test_entry),
                       content_type="application/json")
        response = test_user.get(
            '/api/v1/entries/2', data=json.dumps(test_entry), content_type='application/json')
