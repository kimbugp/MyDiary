...
class dboperations():
    ...
    def make_an_entry(self, user_id, entry_id, entry_date, entry_name, entry_content):
        ...
        new_entry = ("INSERT INTO entries(entry_id,entry_date,entry_name,entry_content) VALUES(%s,%s,%s,%s)")
    ...
    def get_all_entries(self, user_id, entry_id, entry_date, entry_name, entry_content):
        ...
        self.entry_content = entry_content
        self.user_id = user_id
        all_entries = ("SELECT entry_id,entry_date,entry_name,entry_content FROM entries WHERE user_id=%s")
        dict_cursor.execute(all_entries,(self.user_id))
    ...
    def get_one_entries(self, user_id, entry_id, entry_date, entry_name, entry_content):
        ...
        self.entry_content = entry_content
        self.user_id = user_id
        all_entries = ("SELECT entry_id,entry_date,entry_name,entry_content FROM entries WHERE user_id=%s AND entry_id=%s")
        dict_cursor.execute(all_entries,(self.user_id,self.entry_id))
        entries=dict_cursor.fetchall()
        return entries

    def edit_one_entry(self, user_id, entry_id, entry_date, entry_name, entry_content):
        self.entry_id = entry_id
        self.entry_name = entry_name
        self.entry_date = entry_date
        self.entry_content = entry_content
        self.user_id = user_id
        edit_entries = ("UPDATE entries SET entry_name=%s,entry_content=%s WHERE entry_id=%s AND user_id=%s")
        cursor.execute(edit_entries,(self.entry_name,self.entry_content,self.entry_id,self.user_id))
