...
class all_entries_test(unittest.TestCase):
    ...
    def test_API_can_make_new_entry(self):
        ...
        testing_user = app.test_client(self)
        response = testing_user.post('/entries', data=json.dumps(self.test_entry[0]),
    ...
    def test_get_specific_entry(self):
        ...
        #view added entry
        id = {'entry_id': 1}
        response = testing_user.get('/entries/{}'.format(id['entry_id']))
        self.assertEqual(response.status_code, 200)
    ...
    def test_null_entry_(self):
        ...
        response = testing_user.put('entries/1', data=json.dumps(self.test_entry[0]),
                                    content_type='application/json')
